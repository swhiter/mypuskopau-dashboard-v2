<template>
  <ModalBaseWithHeader :title="title" :has-save-button="!readonly" form="update-staff" :loading="modalLoading"
    :button-loading="buttonLoading" :submitted="submitted">
    <Form id="update-staff" class="form-container" @submit="onSubmit">
      <div class="flex w-4/5">
        <div class="grid grid-cols-2 gap-x-4 w-full">
          ...
          <InputForm name="idCardNumber" v-model="userInfo.nik" :label="t('names.idCardNumber')" bordered
            ...
            :disabled="readonly" />
          <InputForm name="username" v-model="userInfo.userId" :label="t('label.staffId')" bordered is-separate-row
...
const getStaffById = async (): Promise<void> => {
  modalLoading.value = true
  try {
    const response = await staffService.getStaffById(props.id)
